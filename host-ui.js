...
(() => {
  ...
  if (recommendedMemory) recommendedMemory.textContent = '4 GB — recommended';
  ...
  byId('server-players-dialog').querySelector(':scope > .mod-status').textContent =
    'Friends using a signed Swirl invite request access with a private player key. Check the short key fingerprint before approving.';
  let activeId = '';
  ...
  function append(line) {
    ...
    consoleView.textContent = text || 'No console output yet.';
  ...
  async function refresh() {
    try {
      ...
      for (const server of servers) {
        ...
        details.textContent = `${server.version} | port ${server.port} | ${server.whitelist ? 'approved names' : 'invite link'}`;
        ...
        if (activeStates.has(runtime.state)) {
          ...
          remove.addEventListener('click', async () => {
            if (!window.confirm(`Delete ${server.name}? Its folder will be moved to Swirl Trash.`))
              return;
            try {
  ...
  byId('update-server-mods').addEventListener('click', async () => {
    ...
    try {
      ...
      if (
        !window.confirm(
          `Update ${plan.length} server mod${plan.length === 1 ? '' : 's'}?\n\n${lines.join('\n')}\n\nSwirl will back up the server and roll everything back if an update fails.`,
        )
      ) {
