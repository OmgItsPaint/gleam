(() => {
  ...
  async function drawPlayers() { const container = byId('server-players-list'); clear(container); if (!playerServer) return; try { const players = await window.icecream.approvedServerPlayers(playerServer.id); byId('server-players-status').textContent = players.length ? `${players.length} approved name${players.length === 1 ? '' : 's'}.` : 'No approved names yet. Add your own name before joining.'; for (const player of players) { const row = document.createElement('article'); row.className = 'mod-card server-mod-item'; const info = document.createElement('div'); const title = document.createElement('div'); title.className = 'mod-title'; title.textContent = player.name; const detail = document.createElement('div'); detail.className = 'mod-meta'; detail.textContent = player.operator ? 'Operator' : 'Player'; info.append(title, detail); const controls = document.createElement('div'); controls.className = 'player-actions'; const operator = document.createElement('button'); operator.className = 'secondary-action'; operator.textContent = player.operator ? 'Remove operator' : 'Make operator'; operator.addEventListener('click', async () => { operator.disabled = true; try { await window.icecream.setApprovedServerPlayer(playerServer.id, player.name, true, !player.operator); await drawPlayers(); } catch (error) { byId('server-players-status').textContent = friendly(error); operator.disabled = false; } }); controls.append(operator); if (activeStates.has(playerServer.runtime?.state)) { const kick = document.createElement('button'); kick.className = 'secondary-action'; kick.textContent = 'Kick'; kick.addEventListener('click', async () => { try { await window.icecream.serverPlayerAction(playerServer.id, player.name, 'kick'); byId('server-players-status').textContent = `${player.name} was kicked.`; } catch (error) { byId('server-players-status').textContent = friendly(error); } }); const ban = document.createElement('button'); ban.className = 'danger-action'; ban.textContent = 'Ban'; ban.addEventListener('click', async () => { if (!window.confirm(`Ban ${player.name} and remove the name from the approved list?`)) return; try { await window.icecream.serverPlayerAction(playerServer.id, player.name, 'ban'); await drawPlayers(); } catch (error) { byId('server-players-status').textContent = friendly(error); } }); controls.append(kick, ban); } const remove = document.createElement('button'); remove.className = 'danger-action'; remove.textContent = 'Remove'; remove.addEventListener('click', async () => { remove.disabled = true; try { await window.icecream.setApprovedServerPlayer(playerServer.id, player.name, false, false); await drawPlayers(); } catch (error) { byId('server-players-status').textContent = friendly(error); remove.disabled = false; } }); controls.append(remove); row.append(info, controls); container.append(row); } } catch (error) { byId('server-players-status').textContent = friendly(error); } }
