...
class IcecreamEngine {
  ...
  async backupModProfile(id) {
    ...
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').replace(/Z$/, `-${crypto.randomInt(100000, 999999)}Z`); const destination = path.join(this.profileBackupRoot(id), stamp); await this.ensure(path.dirname(destination));
  ...
  safeWorldName(value) { const name = String(value || '').trim(); if (!name || name === '.' || name === '..' || path.basename(name) !== name || /[\\/:*?"<>|\0]/.test(name)) throw new Error('Choose a valid world name without file-system symbols.'); return name.slice(0, 80); }
  async worldContext(profileId, requestedWorld = '') { const profile = (await this.getModProfiles()).find(item => item.id === profileId); if (!profile) throw new Error('That profile was not found.'); const saves = path.join(this.instanceDirectory(profile.gameVersion, profile.id), 'saves'); await this.ensure(saves); if (!requestedWorld) return { profile, saves }; const name = this.safeWorldName(requestedWorld); const entry = (await fsp.readdir(saves, { withFileTypes: true })).find(item => item.isDirectory() && item.name === name); if (!entry) throw new Error('That world was not found in this profile.'); return { profile, saves, name, directory: path.join(saves, name) }; }
  async uniqueWorldName(saves, requested) { const base = this.safeWorldName(requested); let candidate = base; let suffix = 2; while (await this.exists(path.join(saves, candidate))) { candidate = `${base.slice(0, Math.max(1, 76 - String(suffix).length))} (${suffix})`; suffix += 1; } return candidate; }
  async listWorlds(profileId) { const { profile, saves } = await this.worldContext(profileId); const worlds = []; for (const entry of await fsp.readdir(saves, { withFileTypes: true }).catch(() => [])) { if (!entry.isDirectory()) continue; const directory = path.join(saves, entry.name); const stat = await fsp.stat(directory); worlds.push({ name: entry.name, profileId: profile.id, profileName: profile.name, profileVersion: profile.gameVersion, size: await this.directorySize(directory), modifiedAt: stat.mtime.toISOString(), valid: await this.exists(path.join(directory, 'level.dat')) }); } return worlds.sort((a, b) => b.modifiedAt.localeCompare(a.modifiedAt)); }
  async copyWorld(sourceProfileId, worldName, targetProfileId, requestedName = '') { if (sourceProfileId === targetProfileId) throw new Error('Choose a different destination profile, or use Duplicate.'); const source = await this.worldContext(sourceProfileId, worldName); const target = await this.worldContext(targetProfileId); await this.backupModProfile(targetProfileId); const name = await this.uniqueWorldName(target.saves, requestedName || source.name); await fsp.cp(source.directory, path.join(target.saves, name), { recursive: true, errorOnExist: true }); return { name, sourceVersion: source.profile.gameVersion, targetVersion: target.profile.gameVersion, versionChanged: source.profile.gameVersion !== target.profile.gameVersion };
  }
  async duplicateWorld(profileId, worldName) { const context = await this.worldContext(profileId, worldName); await this.backupModProfile(profileId); const name = await this.uniqueWorldName(context.saves, `${context.name} Copy`); await fsp.cp(context.directory, path.join(context.saves, name), { recursive: true, errorOnExist: true }); return { name }; }
  async renameWorld(profileId, worldName, requestedName) { const context = await this.worldContext(profileId, worldName); const name = this.safeWorldName(requestedName); if (name === context.name) return { name }; if (await this.exists(path.join(context.saves, name))) throw new Error('A world with that name already exists in this profile.'); await this.backupModProfile(profileId); await fsp.rename(context.directory, path.join(context.saves, name)); return { name }; }
  async deleteWorld(profileId, worldName) { const context = await this.worldContext(profileId, worldName); await this.backupModProfile(profileId); const destination = path.join(this.root, 'trash', 'worlds', `${profileId}-${Date.now()}-${context.name}`); await this.ensure(path.dirname(destination)); await fsp.rename(context.directory, destination); return { name: context.name, recoverableAt: destination }; }
  async exportWorld(profileId, worldName, destinationRoot) { const context = await this.worldContext(profileId, worldName); const root = path.resolve(String(destinationRoot || '')); if (!root) throw new Error('Choose a destination folder.'); await this.ensure(root); const name = await this.uniqueWorldName(root, context.name); const destination = path.join(root, name); await fsp.cp(context.directory, destination, { recursive: true, errorOnExist: true }); return { name, destination }; }
  async importWorld(profileId, sourceDirectory) { const source = path.resolve(String(sourceDirectory || '')); if (!await this.exists(path.join(source, 'level.dat'))) throw new Error('Choose a Minecraft world folder containing level.dat.'); const context = await this.worldContext(profileId); await this.backupModProfile(profileId); const name = await this.uniqueWorldName(context.saves, path.basename(source)); const destination = path.join(context.saves, name); await fsp.cp(source, destination, { recursive: true, errorOnExist: true }); return { name, destination }; }
