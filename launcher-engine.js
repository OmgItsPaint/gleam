...
class IcecreamEngine {
  ...
  async createModProfile(name, gameVersion, sourceProfileId = '') {
    ...
    const profile = { id, name: title, gameVersion, autoSync: (await this.getSettings()).autoUpdate === true, createdAt: new Date().toISOString(), mods: [] }; const migration = { copied: [], skipped: [] };
    try {
      ...
      if (!sourceProfile) await this.atomicWrite(path.join(target, 'mods', 'icecream-mods.json'), '[]');
      else {
  ...
  async diagnostics() { const profiles = await this.getModProfiles(); const settings = await this.getSettings(); const java = {}; for (const major of [8, 17, 21, 25]) { try { java[major] = { available: true, path: this.findJava(major) }; } catch { java[major] = { available: false }; } } return { launcher: LAUNCHER_NAME, version: '1.0.0', platform: process.platform, architecture: process.arch, node: process.version, totalMemoryGiB: Math.round(os.totalmem() / 1024 ** 3), recommendedMemoryGiB: this.memoryGiB(), java, profiles: profiles.map(profile => ({ id: profile.id, name: profile.name, gameVersion: profile.gameVersion, requiredJava: fallbackJavaMajor(profile.gameVersion), mods: (profile.mods || []).length, autoSync: profile.autoSync === true, bundledClientAvailable: fs.existsSync(path.join(__dirname, 'bundled-mods', `swirl-client-${profile.gameVersion}.jar`)) })), settings: { autoUpdate: settings.autoUpdate, experimentalVersions: settings.experimentalVersions === true, lastVersion: settings.lastVersion || '', fabricLoaderVersions: settings.fabricLoaderVersions || {} }, dataDirectory: this.root }; }
