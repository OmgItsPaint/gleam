...
if (SMOKE_TEST && process.env.SWIRL_SMOKE_USER_DATA)
  app.setPath('userData', process.env.SWIRL_SMOKE_USER_DATA);
...
function requireProfileId(id) {
  if (!PROFILE_ID_PATTERN.test(String(id))) throw new Error('Choose a valid Swirl profile.');
  return String(id);
}
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    minWidth: 900,
    minHeight: 650,
    icon: path.join(__dirname, 'assets', 'swirl-logo.svg'),
    frame: false,
...
ipcMain.handle('offline-player', async (_, requestedName = '') => {
  const username = String(requestedName || '').trim() || `Swirl${crypto.randomInt(1000, 9999)}`;
...
ipcMain.handle('export-player-identity', async (_, passphrase) => {
  ...
  const result = await dialog.showSaveDialog(mainWindow, {
    title: 'Save encrypted Swirl identity recovery',
    defaultPath: 'Swirl-player-identity.swirlidentity',
    filters: [{ name: 'Swirl identity recovery', extensions: ['swirlidentity'] }],
  });
...
ipcMain.handle('import-player-identity', async (_, passphrase) => {
  ...
  const result = await dialog.showOpenDialog(mainWindow, {
    title: 'Open Swirl identity recovery',
    properties: ['openFile'],
    filters: [{ name: 'Swirl identity recovery', extensions: ['swirlidentity', 'json'] }],
  });
...
ipcMain.handle('fetch-versions', async () => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        'Swirl could not reach Minecraft services and has no saved versions yet. Check your connection and try again.',
      );
    }
  }
  const settings = await engine.getSettings();
...
ipcMain.handle('update-mods', async (_, gameVersion, profileId) =>
  engine.updateAllMods(requireVersion(gameVersion), requireProfileId(profileId)),
);
...
ipcMain.handle('repair-mod-profile', async (_, id) =>
  engine.repairModProfile(requireProfileId(id)),
);
...
ipcMain.handle('backup-mod-profile', async (_, id) =>
  engine.backupModProfile(requireProfileId(id)),
);
...
ipcMain.handle('save-diagnostics', async () => {
  ...
  const result = await dialog.showSaveDialog(mainWindow, {
    title: 'Save Swirl troubleshooting report',
    defaultPath: `Swirl-support-${new Date().toISOString().slice(0, 10)}.json`,
...
ipcMain.handle('create-server', async (_, name, version, port, options = {}) =>
  servers.create(name, requireVersion(version), port, {
    ...
    hostIdentity: await identities.info(),
  }),
);
...
ipcMain.handle('save-server-invite', async (_, id) => {
  ...
  const result = await dialog.showSaveDialog(mainWindow, {
    title: 'Save Swirl server invite',
    defaultPath: `${server.name.replace(/[^a-z0-9_-]+/gi, '-')}.swirlinvite`,
    filters: [{ name: 'Swirl server invite', extensions: ['swirlinvite'] }],
  });
  if (result.canceled || !result.filePath) return { saved: false };
...
ipcMain.handle('open-server-invite-file', async () => {
  const result = await dialog.showOpenDialog(mainWindow, {
    title: 'Open Swirl server invite',
    properties: ['openFile'],
    filters: [{ name: 'Swirl server invite', extensions: ['swirlinvite', 'txt'] }],
  });
...
ipcMain.handle('backup-server', async (_, id) =>
  servers.backup(requireProfileId(id), (await engine.getSettings()).backupRetention),
);
...
ipcMain.handle('restore-server-backup', async (_, id, backupId) =>
  servers.restoreBackup(requireProfileId(id), String(backupId || '')),
);
...
ipcMain.handle('install-server-mod', async (_, id, projectId, versionId = '') =>
  servers.installMod(requireProfileId(id), String(projectId || ''), String(versionId || '')),
);
