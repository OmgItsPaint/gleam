...
class IcecreamServerEngine {
  ...
  parseProperties(text) { const values = {}; for (const line of String(text || '').split(/\r?\n/)) { const trimmed = line.trim(); if (!trimmed || trimmed.startsWith('#') || !trimmed.includes('=')) continue; const index = trimmed.indexOf('='); values[trimmed.slice(0, index)] = trimmed.slice(index + 1); } return values; }
  async getProperties(id) { const server = (await this.rawList()).find(item => item.id === id); if (!server) throw new Error('Server not found.'); const values = this.parseProperties(await fsp.readFile(path.join(this.dir(id), 'server.properties'), 'utf8')); return { motd: values.motd || 'Swirl private server', maxPlayers: Number(values['max-players']) || 12, gamemode: values.gamemode || 'survival', difficulty: values.difficulty || 'easy', pvp: values.pvp !== 'false', commandBlocks: values['enable-command-block'] === 'true', viewDistance: Number(values['view-distance']) || 10, simulationDistance: Number(values['simulation-distance']) || 8 }; }
  async setProperties(id, changes = {}) { if (this.running.has(id)) throw new Error('Stop the server before changing its settings.'); return this.withServerLock(id, async () => { const server = (await this.rawList()).find(item => item.id === id); if (!server) throw new Error('Server not found.'); const allowedModes = new Set(['survival', 'creative', 'adventure', 'spectator']); const allowedDifficulty = new Set(['peaceful', 'easy', 'normal', 'hard']); const values = { motd: String(changes.motd || 'Swirl private server').replace(/[\r\n]/g, ' ').trim().slice(0, 100), 'max-players': Math.max(1, Math.min(100, Number(changes.maxPlayers) || 12)), gamemode: allowedModes.has(changes.gamemode) ? changes.gamemode : 'survival', difficulty: allowedDifficulty.has(changes.difficulty) ? changes.difficulty : 'easy', pvp: changes.pvp !== false, 'enable-command-block': changes.commandBlocks === true, 'view-distance': Math.max(2, Math.min(32, Number(changes.viewDistance) || 10)), 'simulation-distance': Math.max(2, Math.min(32, Number(changes.simulationDistance) || 8)) }; await this.backup(id, this.backupRetention, true); const file = path.join(this.dir(id), 'server.properties'); const original = await fsp.readFile(file, 'utf8'); const seen = new Set(); const lines = original.split(/\r?\n/).filter((line, index, list) => index < list.length - 1 || line).map(line => { const trimmed = line.trim(); if (!trimmed || trimmed.startsWith('#') || !trimmed.includes('=')) return line; const key = trimmed.slice(0, trimmed.indexOf('=')); if (!(key in values)) return line; seen.add(key); return `${key}=${values[key]}`; }); for (const [key, value] of Object.entries(values)) if (!seen.has(key)) lines.push(`${key}=${value}`); await this.atomicWrite(file, `${lines.join('\n')}\n`); return this.getProperties(id); }); }
