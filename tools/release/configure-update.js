...
const publicKey = String(
  process.env.GLEAM_UPDATE_PUBLIC_KEY || process.env.SWIRL_UPDATE_PUBLIC_KEY || '',
)
  .replace(/\\n/g, '\n')
  .trim();
...
if (!publicKey.includes('BEGIN PUBLIC KEY'))
  throw new Error('GLEAM_UPDATE_PUBLIC_KEY must contain the Ed25519 public key in PEM format.');
const config = {
  format: 2,
  enabled: true,
...
fs.writeFileSync(
  path.join(__dirname, '..', '..', 'config', 'update-config.json'),
